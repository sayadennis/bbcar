...
class ScanMap(nn.Module):
    ...
    def __autograd__(self,epoch):
        ...
        if self.y is not None:
            ...
            y_pairs = torch.cat([torch.repeat_interleave(self.y, self.ntr, dim=0).reshape(1,-1), self.y.repeat(1, self.ntr)], dim=0)
            fourclass = []
            for i in range(y_pairs.shape[1]):
                if torch.all(torch.eq(y_pairs[:,i],torch.tensor([0,0]).to(self.device))):
                    fourclass.append(0)
                elif torch.all(torch.eq(y_pairs[:,i],torch.tensor([1,0]).to(self.device))):
                    fourclass.append(1)
                elif torch.all(torch.eq(y_pairs[:,i],torch.tensor([0,1]).to(self.device))):
                    fourclass.append(2)
                elif torch.all(torch.eq(y_pairs[:,i],torch.tensor([1,1]).to(self.device))):
                    fourclass.append(3)
            ...
            self.celoss = self.loss_cls(
                ...
                torch.tensor(fourclass).to(self.device) # [sample_ix]
            ) # y_pairs_onehot
