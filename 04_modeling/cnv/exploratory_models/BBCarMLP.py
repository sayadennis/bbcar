...
class BBCarMLP(torch.nn.Module):
    ...
    def __initfact__(self, X, Xval, y, yval):
        ...
        if self.contrastive:
            self.ix_pairs = self.get_pairs(range(self.X.shape[0]))
            print('There are %s pairs with %s samples to choose from\n' % (len(self.ix_pairs), self.X.shape[0]))
            self.ydist = torch.tensor([(-2)*abs(self.y[i]-self.y[j])+1 for (i,j) in self.ix_pairs], dtype=float).to(self.device)
            self.ydist = torch.tensor(np.zeros(shape=(len(self.y), len(self.y)), dtype=float)).to(self.device)
            for i in range(X.shape[0]):
                for j in range(X.shape[0]):
                    self.ydist[i,j] = (-2)*abs(self.y[i]-self.y[j])+1 # -1 if different class, 1 if same class 
                    # if different class, subtract the distance from loss (want to maximize distance) 
                    # if same class, add the distance to the loss (want to minimize distance) 
    ...
    def __autograd__(self,epoch):
        ...
        if self.y is not None:
            ...
            if self.contrastive=='euclidean':
                ...
                for (i,j) in self.ix_pairs:
                    contloss += self.ydist[i,j] * torch.cdist(torch.reshape(self.finallayer[i,],(1,-1)), torch.reshape(self.finallayer[j,],(1,-1)))[0][0]
                    ...
                    # elif dist==1:
                    #     contloss -= torch.cdist(torch.reshape(self.finallayer[i,],(1,-1)), torch.reshape(self.finallayer[j,],(1,-1)))[0][0]
                l += self.wcont * (contloss / len(self.ix_pairs))
