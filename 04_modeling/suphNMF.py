...
class suphNMF(nn.Module):
    def __init__(self, X1, X2, y=None, n_components=5, seed=42):
    ...
    def crossval_fit(self,
            n_iters=[1000,2000,3000], lrs=[1e-4,1e-3,1e-2], weight_decays=[1e-3, 1e-4, 1e-5], 
            clf_weights=[0,1e-1,1e+0,1e+1,1e+2], ortho_weights=[0,1e-1,1e+0,1e+1,1e+2]
        ):
        ...
        self.cv_performance = pd.DataFrame(
            index=pd.MultiIndex.from_tuples(param_tuples, names=('n_iter', 'lr', 'weight_decay', 'clf_weight', 'ortho_weight')), 
            columns=['performance']
        )
        ...
        for n_iter, lr, weight_decay, w_clf, w_ortho in param_tuples:
            ...
            for i, (train_index, val_index) in enumerate(skf.split(self.X1, self.y)):
                ...
                self.fit(
                    ...
                    n_iter=n_iter, lr=lr, weight_decay=weight_decay, clf_weight=w_clf, ortho_weight=w_ortho
                )
            ...
            self.cv_performance.to_csv('/home/srd6051/test_cv_cleaned_methods.csv')  # TODO: fix hard-coded path
        ...
        best_params = {k : v for k, v in zip(self.cv_performance.index.names, best_params_tuple[0])}
    ...
    def fit(self, X1, X2, y, n_iter, lr, weight_decay, clf_weight, ortho_weight):
        ...
        # NMF matrices
        self.W = torch.nn.Parameter(torch.normal(mean=3., std=1.5, size=(X1.shape[0], self.n_components)).clamp(min=1e-5))
        self.H1 = torch.nn.Parameter(torch.normal(mean=3., std=1.5, size=(self.n_components, X1.shape[1])).clamp(min=1e-5))
        self.H2 = torch.nn.Parameter(torch.normal(mean=3., std=1.5, size=(self.n_components, X2.shape[1])).clamp(min=1e-5))
        ...
        for i in range(self.n_iter):
            ...
            loss_recon1 = self.recon_loss_func(torch.mm(self.W, self.H1), X1) / X1.shape[1]  # div by number of features (align scale between X1 and X2) 
            loss_recon2 = self.recon_loss_func(torch.mm(self.W, self.H2), X2) / X2.shape[1]
            ...
            if self.supervised:
                self.clf_roc_record.append(metrics.roc_auc_score(y.detach().numpy(), y_score.detach().numpy()))
    ...
    def evaluate(self, X1, X2, y):
        ...
        if self.supervised:
            ...
            y_pred = (y_score.clone().detach()>0.5).int()
            loss_clf = self.clf_loss_func(y_out, y)
            clf_roc = metrics.roc_auc_score(y.detach().numpy(), y_score.detach().numpy())
        else:
            y_out = self.predict(W)
            y_score = torch.sigmoid(y_out).ravel()
            y_pred = (y_score.clone().detach()>0.5).int()
        ...
        loss_overall = loss_recon1 + loss_recon2 + self.clf_weight * loss_clf + self.ortho_weight * loss_W_ortho
    ...
    def transform(self, X1, X2, y=None):
        ...
        W = torch.nn.Parameter(
            torch.normal(mean=3., std=1.5, size=(X1.shape[0], self.n_components)).clamp(min=1e-5)
        )
