...
class suphNMF(nn.Module):
    def __init__(self, X1, X2, y, n_components=k, lr=1e-3, n_iter=2000, weight_decay=1e-4, clf_weight=0.1, seed=42):
    ...
    def plus(self):
        self.W = self.W.clamp(min=1e-5)
        self.H1 = self.H1.clamp(min=1e-5)
        self.H2 = self.H2.clamp(min=1e-5)

    def fit(self):
        self.optimizer = optim.Adam(self.parameters(), lr=self.lr, weight_decay=self.weight_decay)
        self.clf_record = []
        self.stability_record = []
        self.reconloss_record = []
        self.loss_record = []
        for i in range(self.n_iter):
            ...
            self.loss_recon1 = self.recon_loss_func(torch.mm(self.W, self.H1), self.X1).item()
            self.loss_recon2 = self.recon_loss_func(torch.mm(self.W, self.H2), self.X2).item()
            # Add L2 (or KL) regularization for orthogonality between components
            #self.loss_W_ortho = # TODO: implement orthogonality regularization
            ...
            ## Backprop & step
            loss = self.loss_recon1 + self.loss_recon2 + self.clf_weight * self.loss_clf
            loss.backward()
            self.optimizer.step()
            ...
            self.plus()
            ## Record performance
            self.record_performance(y_pred)
    ...
    def record_performance(self, y_pred):
        """
        Record performance of classification, stability (Silhouette score), factorization (reconstruction) loss.
        """
        self.clf_record.append(metrics.roc_auc_score(self.y.detach().numpy(), y_pred.detach().numpy()))
