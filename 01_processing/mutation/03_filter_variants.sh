...
function split_dp_filter() {
    ...
    bcftools norm --multiallelics -both \
        --output-type v \
        --output ${dout}/${sampleid}_split.vcf \
...
if [[ " ${tissue[*]} " =~ " ${sampleid} " ]]; then
    ...
    split_dp_filter ${sampleid} 'tissue_only'
else
...
if [[ " ${germline[*]} " =~ " ${sampleid} " ]]; then
    ...
    split_dp_filter ${sampleid} 'tissue_normal'
    echo "########## Filtering variants on germline sample ${sampleid} ##########"
    split_dp_filter ${sampleid} 'germline_only'
else
