...
anno = pd.read_csv(
    f"{dn}/04_ml_features/02_concat_annovar_features/annovar_features_all.csv"
)  # annovar_features_all_bbcarpon.csv
...
for var_id in data.index:
    if var_id not in func_dict["Func.refGene"]["intronic"]:
        gene = anno.iloc[anno.var_id.values == var_id, :]["Gene.refGene"].iloc[0]
        var_positive_samples = (
            data.loc[var_id].iloc[data.loc[var_id, :].values == 1].index
        )
        ct_gene.loc[gene, var_positive_samples] += 1
...
for var_id in data.index:
    if (var_id in del_dict["Polyphen2_HDIV_pred"]["D"]) | (
        var_id in del_dict["Polyphen2_HDIV_pred"]["P"]
    ):
        gene = anno.iloc[anno.var_id.values == var_id, :]["Gene.refGene"].iloc[0]
        var_positive_samples = (
            data.loc[var_id].iloc[data.loc[var_id, :].values == 1].index
