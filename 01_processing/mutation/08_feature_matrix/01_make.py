...
# Create empty matrix
X = pd.DataFrame(
    None, index=all_var.sample_id.unique(), columns=all_var.var_id.unique()
)

# First, populate the paired calls ("ground truth" from tumor_normal and germline_only)
soma = (
    all_var.iloc[all_var.source.values == "tumor_normal"]
    .groupby(["sample_id", "var_id"])
    .size()
)
germ = (
    all_var.iloc[all_var.source.values == "germline_only"]
    .groupby(["sample_id", "var_id"])
    .size()
)
...
pred_soma = unk.iloc[
    [bool(y_nonmatched.loc[var_id, "somatic"]) for var_id in unk.var_id.values], :
]
