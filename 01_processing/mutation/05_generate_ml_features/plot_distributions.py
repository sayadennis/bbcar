...
for func_type in ["Func.refGene", "Func.knownGene", "Func.ensGene"]:
    ...
    miss_rate[func_type] = pd.DataFrame(
        index=unique_funcs, columns=["SIFT_score", "Polyphen2_HDIV_score", "CADD_raw"]
    )
    for unique_func in unique_funcs:
        miss_rate[func_type].loc[unique_func, :] = (
            pd.isnull(
                test_features.iloc[test_features[func_type].values == unique_func, :]
            ).sum()
            / test_features.iloc[
                test_features[func_type].values == unique_func, :
            ].shape[0]
        )
...
for func_type in ["Func.refGene", "Func.knownGene", "Func.ensGene"]:
    ...
    for i in range(3):
        ...
        axs[i].bar(
            np.arange(miss_rate[func_type].shape[0]), miss_rate[func_type][score].values
        )
        axs[i].set_xticks(np.arange(len(miss_rate[func_type].index)))
        axs[i].set_xticklabels(
            miss_rate[func_type].index, rotation=30, ha="right", fontsize=10
        )
...
for i in range(1, plot_features.shape[1]):
    ...
    if var_name in categorical:
        miss_rate = (
            100 * pd.isnull(plot_features[var_name]).sum() / plot_features.shape[0]
        )
        ...
        axs[(i - 1) // 6, (i - 1) % 6].set_xticklabels(
            cts.index, rotation=30, ha="right"
        )  # , fontsize=14
        axs[(i - 1) // 6, (i - 1) % 6].set_title(
            f"{var_name} ({miss_rate:.1f}% missing)", fontsize=16
        )
    else:
        miss_rate = (
            100 * pd.isnull(plot_features[var_name]).sum() / plot_features.shape[0]
        )
        axs[(i - 1) // 6, (i - 1) % 6].hist(
            plot_features.iloc[~pd.isnull(plot_features[var_name]).values, i]
            .to_numpy()
            .ravel()
        )
        axs[(i - 1) // 6, (i - 1) % 6].set_title(
