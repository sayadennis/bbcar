...
## Data
din = (
    f"/projects/b1131/saya/bbcar/exploratory_winham_filter/{filter_type}/04_ml_features"
)
...
## Model
model_dir = (
    "/projects/b1131/saya/bbcar/"
    f"model_interpretations/exploratory_winham_filter/{filter_type}"
)
...
pd.DataFrame(y_nonmatched, index=X_nonmatched.index, columns=["somatic"]).to_csv(
    f"{dout}/nonmatched_{pon_source}.csv"
)

pd.DataFrame(y_matched_pred, index=X_matched.index, columns=["somatic"]).to_csv(
    f"{dout}/matched_{pon_source}.csv"
)
