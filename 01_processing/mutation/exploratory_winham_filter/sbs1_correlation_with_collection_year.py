...
for filter_type in ["liberal", "classical", "strict"]:
    ...
    for sigtype in ["SBS96", "DBS78", "ID83"]:
        ...
        samples_sig = pd.read_csv(
            f"{dn}/{sigtype}/Samples.txt", sep="\t", index_col=0
        ).T
        opt_num_sig = pd.read_csv(
            (
                f"{dn}/{sigtype}/Suggested_Solution/{sigtype}_De-Novo_Solution"
        ...
        opt_solution = pd.read_csv(
            (
                f"{dn}/{sigtype}/All_Solutions/{sigtype}_{opt_num_sig}_Signatures"
        ...
        decomposed_sig = pd.read_csv(
            (
                f"{dn}/{sigtype}/Suggested_Solution/COSMIC_{sigtype}_Decomposed_Solution"
...
clin = pd.read_csv(
    f"{datadir}/00_raw/BBCaRDatabaseNU09B2_DATA_2023-04-04_0551.csv", index_col=0
)
...
scatter_ylim = (
    -15,
    np.max(sig_per_sample["no filter"]["SBS96"]["COSMIC"]["SBS1"]) + 30,
)
...
for i, filter_type in enumerate(["no filter", "liberal", "classical", "strict"]):
    ...
    axs[i].set_title(
        f"Filter: {filter_type.upper()} (r={r.statistic:.2f}, p={r.pvalue:.3f})"
    )
