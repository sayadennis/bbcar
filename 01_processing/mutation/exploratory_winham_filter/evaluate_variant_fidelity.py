...
germline_vcfs = glob.glob(
    (
        ...
        "/02_variant_calls/germline_only/*_DPfiltered.vcf"
    )
)
germline_sample_ids = [
...
for filter_type in ["liberal", "classical", "strict"]:
    ...
    din = (
        ...
        f"/{filter_type}/02_variant_calls/tumor_only"
    )
...
for filter_type in ["liberal", "classical", "strict"]:
    ...
    for sample_id in germline_sample_ids:
        ...
        din = (
            f"/projects/b1131/saya/bbcar/exploratory_winham_filter"
            f"/{filter_type}/02_variant_calls/tumor_normal"
        )
        if len(glob.glob(f"{din}/{sample_id}_*")) == 0:
            continue
        fin = glob.glob(f"{din}/{sample_id}_*")[0]
...
performance = pd.DataFrame(
    columns=["FILTER", "F1", "TP", "FP", "FN", "TP_STD", "FP_STD", "FN_STD"]
)
for filter_type in ["liberal", "classical", "strict"]:
    tp = [
        len(
            called[filter_type][sample_id].intersection(
                ground_truth[filter_type][sample_id]
            )
        )
        for sample_id in germline_sample_ids
    ]
    ...
    tp_std = np.std(tp)
    fp = [
        len(called[filter_type][sample_id] - ground_truth[filter_type][sample_id])
        for sample_id in germline_sample_ids
    ]
    fp_mean = np.mean(fp)
    fp_std = np.std(fp)
    fn = [
        len(ground_truth[filter_type][sample_id] - called[filter_type][sample_id])
        for sample_id in germline_sample_ids
    ]
    fn_mean = np.mean(fn)
    fn_std = np.std(fn)
    ...
    performance = pd.concat(
        (
            ...
            pd.DataFrame(
                ...
                columns=[0],
                index=["FILTER", "F1", "TP", "FP", "FN", "TP_STD", "FP_STD", "FN_STD"],
            ).T,
        ),
        axis=0,
    )
...
# germline_vcfs = glob.glob(
#     f'/projects/b1131/saya/bbcar/data/02a_mutation/02_variant_calls/germline_only/*DPfiltered.vcf'
# )
...
#     din = (f'/projects/b1131/saya/bbcar/exploratory_winham_filter"
#            f'/{filter_type}/02_variant_calls/tumor_only')
#     # dout = f'/projects/b1131/saya/bbcar/exploratory_winham_filter/{filter_type}/evaluate_calls'
#     for sample_id in germline_sample_ids:
