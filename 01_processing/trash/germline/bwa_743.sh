...
#SBATCH --output=/home/srd6051/bbcar/out/bwa_743_germline.out
#SBATCH --job-name=bwa_743_germline
...
R1=/projects/b1131/saya/bbcar/raw/germline/743/743_S17_L004_R1.fastq.gz
R2=/projects/b1131/saya/bbcar/raw/germline/743/743_S17_L004_R2.fastq.gz

header=$(zcat $R1 | head -n 1)
id=$(echo $header | head -n 1 | cut -f 3-4 -d':' | sed 's/@//' | sed 's/:/_/g')
echo '@RG\tID:$id\tSM:743_germline\tLB:library1\tPL:ILLUMINA\tPU:$id'

bwa mem -M -t 24 -R $(echo '@RG\tID:$id\tSM:743_germline\tLB:library1\tPL:ILLUMINA\tPU:$id') $FA $R1 $R2 > $interim/743_1.sam

$pic SortSam I=$interim/743_1.sam O=$interim/743_1_sorted.bam SORT_ORDER=coordinate


$pic MergeSamFiles CREATE_INDEX=true I=$interim/743_1_sorted.bam O=$interim/743_final_sorted.bam USE_THREADING=true

rm -f $interim/743_1.sam
rm -f $interim/743_1_sorted.bam

$pic MarkDuplicates I=$interim/743_final_sorted.bam O=$interim/743_dup.bam M=$metrics/743_germline_reads.mdup.metrics.txt

rm -f $interim/743_final_sorted.bam
rm -f $interim/743_final_sorted.bai

gatk BaseRecalibrator -I $interim/743_dup.bam -R $FA --known-sites $dbsnp --known-sites $gold1000Indel -O $rec_tables/743_germline_recal_data.table

samtools index $interim/743_dup.bam

gatk ApplyBQSR -R $FA -I $interim/743_dup.bam --bqsr-recal-file $rec_tables/743_germline_recal_data.table -L $interval -O $aligned/743_bqsr.bam
