...
#SBATCH --output=/home/srd6051/bbcar/out/bwa_129_tissue.out
#SBATCH --job-name=bwa_129_tissue
...
R1=/projects/b1131/saya/bbcar/raw/tissue/129/129_GAATCTG_L001_R2.fastq.gz
R2=/projects/b1131/saya/bbcar/raw/tissue/129/129_GAATCTG_L001_R1.fastq.gz

header=$(zcat $R1 | head -n 1)
id=$(echo $header | head -n 1 | cut -f 3-4 -d':' | sed 's/@//' | sed 's/:/_/g')
echo '@RG\tID:$id\tSM:129_tissue\tLB:library1\tPL:ILLUMINA\tPU:$id'

bwa mem -M -t 24 -R $(echo '@RG\tID:$id\tSM:129_tissue\tLB:library1\tPL:ILLUMINA\tPU:$id') $FA $R1 $R2 > $interim/129_1.sam

$pic SortSam I=$interim/129_1.sam O=$interim/129_1_sorted.bam SORT_ORDER=coordinate


$pic MergeSamFiles CREATE_INDEX=true I=$interim/129_1_sorted.bam O=$interim/129_final_sorted.bam USE_THREADING=true

rm -f $interim/129_1.sam
rm -f $interim/129_1_sorted.bam

$pic MarkDuplicates I=$interim/129_final_sorted.bam O=$interim/129_dup.bam M=$metrics/129_tissue_reads.mdup.metrics.txt

rm -f $interim/129_final_sorted.bam
rm -f $interim/129_final_sorted.bai

gatk BaseRecalibrator -I $interim/129_dup.bam -R $FA --known-sites $dbsnp --known-sites $gold1000Indel -O $rec_tables/129_tissue_recal_data.table

samtools index $interim/129_dup.bam

gatk ApplyBQSR -R $FA -I $interim/129_dup.bam --bqsr-recal-file $rec_tables/129_tissue_recal_data.table -L $interval -O $aligned/129_bqsr.bam
