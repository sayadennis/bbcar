...
#SBATCH --output=/home/srd6051/bbcar/out/bwa_1389_tissue.out
#SBATCH --job-name=bwa_1389_tissue
...
R1=/projects/b1131/saya/bbcar/raw/tissue/1389/1389_GAGCTGAA_L005_R1.fastq.gz
R2=/projects/b1131/saya/bbcar/raw/tissue/1389/1389_GAGCTGAA_L005_R2.fastq.gz

header=$(zcat $R1 | head -n 1)
id=$(echo $header | head -n 1 | cut -f 3-4 -d':' | sed 's/@//' | sed 's/:/_/g')
echo '@RG\tID:$id\tSM:1389_tissue\tLB:library1\tPL:ILLUMINA\tPU:$id'

bwa mem -M -t 24 -R $(echo '@RG\tID:$id\tSM:1389_tissue\tLB:library1\tPL:ILLUMINA\tPU:$id') $FA $R1 $R2 > $interim/1389_1.sam
...
$pic MarkDuplicates I=$interim/1389_final_sorted.bam O=$interim/1389_dup.bam M=$metrics/1389_tissue_reads.mdup.metrics.txt

rm -f $interim/1389_final_sorted.bam
rm -f $interim/1389_final_sorted.bai

gatk BaseRecalibrator -I $interim/1389_dup.bam -R $FA --known-sites $dbsnp --known-sites $gold1000Indel -O $rec_tables/1389_tissue_recal_data.table

samtools index $interim/1389_dup.bam

gatk ApplyBQSR -R $FA -I $interim/1389_dup.bam --bqsr-recal-file $rec_tables/1389_tissue_recal_data.table -L $interval -O $aligned/1389_bqsr.bam
