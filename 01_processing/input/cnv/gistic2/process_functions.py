...
def generate_gistic_features(gistic_dir, q_thres=0.05):
    ...
    The returned object is a set of Pandas DataFrames stored in a dictionary with the following keys:
        - 'genecopy'
        - 'genethres'
        - 'regcopy'
        - 'regthres'
    ...
    gene_copy = pd.read_csv(f'{gistic_dir}/all_data_by_genes.txt', sep='\t', index_col=0)
    gene_copy = gene_copy.T.iloc[2:,:] # remove 'Gene ID' and 'Cytoband'
    # all_thresholded.by_genes.txt file
    gene_thres = pd.read_csv(f'{gistic_dir}/all_thresholded.by_genes.txt', sep='\t', index_col=0)
    ...
    patids = reg.iloc[:,['_' in x for x in reg.columns]].columns
    ...
    for cytoband in cytobands:
        for patid in patids:
            cn = reg.iloc[reg['Descriptor'].values==cytoband, [x==patid for x in reg.columns]] # CNA for that patient & cytoband
            ...
            cyto_copy.loc[patid,cytoband] = cn_copy.iloc[np.argmax(abs(cn_copy)),:].values[0]
            # get threshold value and save it to cyto_thres
            if np.any(['Amplification' in x for x in cn_thres.index]):
                thres_amp = cn_thres.iloc[['Amplification' in x for x in cn_thres.index],:]
    ...
    feature_dict = {
        'genecopy' : gene_copy,
        'genethres' : gene_thres,
        'regcopy' : reg_copy,
        'regthres' : reg_thres,
        'cytocopy' : cyto_copy,
        'cytothres_amp' : cyto_thres_amp,
        'cytothres_del' : cyto_thres_del
    }
    # change the indices from sample names (e.g. '1004_Tissue') to study IDs (e.g. 1004)
    for key in feature_dict.keys():
        feature_dict[key].index = [int(x.split('_')[0]) for x in feature_dict[key].index]
        feature_dict[key].sort_index(axis=0, inplace=True)
    return feature_dict
...
def lesions_to_structured_regions(lesion_fn):
    ...
    The columns are as follows:
        - 'chrom' : string that looks like 'chr1' --string is better than integer because 'chrX' exists
    ...
    regions = pd.DataFrame(
        index=lesions.iloc[['CN' not in x for x in lesions['Unique Name'].values],:]['Unique Name'].values, # region names 
        columns=['chrom', 'start', 'end']
    )
    for region in regions.index:
        peak_limits = lesions.iloc[lesions['Unique Name'].values==region, [x=='Wide Peak Limits' for x in lesions.columns]].values[0][0].strip()
        regions.loc[region,'chrom'] = peak_limits.split(':')[0] # get the string "chr<num>"
        regions.loc[region,'start'] = int(peak_limits.split(':')[1].split('-')[0])
        regions.loc[region,'end'] = int(peak_limits.split(':')[1].split('-')[1].split('(')[0])
    return regions


def generate_ovelap_mx(reg1, reg2):
    """
    This is a function that takes two structured region files (ref. "lesions_to_structured_regions" above)
        ...
        for i in range(len(samechrom)): # loop through reg2 regions
            if samechrom[i]:
                if (start_contained[i] & end_contained[i]):
                    ...
                    ol.append((reg2['end']-start)[i]) # looking at i'th region of the control 
...
def transform_gatk_to_reg_matrix(reg_table, gatk_dir, sampleset=[]):
    ...
    Descriptions of each argument:
        - reg_table (Pandas DataFrame) : table of regions to consider w/ columns=['chrom','start','end']
    ...
    for fn in glob.glob(gatk_dir + '/*.csv'):
        ...
        if studyix in sampleset:
            ...
            for i in gatk.index:
                ...
                chrom_match = np.array([x == chrom for x in reg_table['chrom'].values]) # boolean as to whether chromosome matches 
                start_include = (start > reg_table['start'].values) & (start < reg_table['end'].values) # boolean as to whether the segment start is included in the region
                end_include = (end > reg_table['start'].values) & (end < reg_table['end'].values) # boolean as to whether the segment start is included in the region
                if np.any((chrom_match & (start_include | end_include))):
                    match_reg = list(reg_table.iloc[(chrom_match & (start_include | end_include))].index)
                    for reg in match_reg:
                        if abs(gatk.loc[i,'MEAN_LOG2_COPY_RATIO']) > abs(mx.loc[studyix, reg]): # only overwrite if absolute aberrant value is bigger 
                            mx.loc[studyix, reg] = gatk.loc[i,'MEAN_LOG2_COPY_RATIO']-1 # -1 because GISTIC and GATK uses different log2 standards
