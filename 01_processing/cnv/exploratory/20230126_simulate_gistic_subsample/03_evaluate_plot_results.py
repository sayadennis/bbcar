...
for i in range(5):
    din = (
        f"/projects/b1131/saya/bbcar/data/02b_cnv"
        f"/20230126_simulate_gistic_subsample/gistic_output/subset{i}"
    )
    ...
    thres = data.iloc[
        np.invert([x.endswith("values") for x in data["Unique Name"].values]),
        : data.shape[1] - 1,
    ]  # last column is all NaN
    ...
    for aber_type in ["Amplification", "Deletion"]:
        df = thres.iloc[
            [x.startswith(aber_type) for x in thres["Unique Name"].values], :
        ]
    ...
    for aber_type in ["Amplification", "Deletion"]:
        ...
        region_aber_cases = aber_thres[aber_type].iloc[
            :, [x.endswith("Tissue") for x in aber_thres[aber_type].columns]
        ]
        region_aber_controls = aber_thres[aber_type].iloc[
            :, [x.endswith("Control") for x in aber_thres[aber_type].columns]
        ]
        # get regions highly aberrated in cases and controls
        freq_thres = 0
        highly_aber_in_cases = set(
            region_aber_cases.sum(axis=1)
            .sort_values(ascending=False)
            .iloc[
                region_aber_cases.sum(axis=1).sort_values(ascending=False).values
                > region_aber_cases.shape[1] * freq_thres
            ]
            .index
        )
        highly_aber_in_controls = set(
            region_aber_controls.sum(axis=1)
            .sort_values(ascending=False)
            .iloc[
                region_aber_controls.sum(axis=1).sort_values(ascending=False).values
                > region_aber_controls.shape[1] * freq_thres
            ]
            .index
        )
    ...
    for aber_type in ["Amplification", "Deletion"]:
        for region_name in selected_regions[aber_type]:
            corresponding_cytoband = data.iloc[
                data["Unique Name"].values == region_name, :
            ].Descriptor.values[0]
            n_samples = (
                aber_thres[aber_type]
                .iloc[:, [x.endswith("Tissue") for x in aber_thres[aber_type].columns]]
                .loc[region_name]
                .sum()
            )
            try:
