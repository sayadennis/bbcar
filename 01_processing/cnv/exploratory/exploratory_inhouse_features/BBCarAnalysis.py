...
def CytobandMatrix(cytoband, cna_dir):
    ...
    Arguments:
    cytoband - Pandas DataFrame with cols "name", "CONTIG", "START", and "END".
    cna_dir - absolute/relative path to the directory that contains all patient's files of CNA.
    Usage:
    ...
    """
    # Generate a list of cytoband names (chrom number + cytoband name e.g. "1p36.33")
    cytoband_names = []
    ...
    for filepath in glob.glob(cna_dir + "/*.csv"):
        ...
        sample_name = filepath.split("/")[-1].split(".")[
            0
        ]  # either "<sampleid>_Tissue" or "<sampleid>_Control"
        # Append new row with all zeros to cna_matrix
        cna_matrix = cna_matrix.append(
            pd.DataFrame(
                np.zeros((1, len(cytoband_names)), dtype=int),
                index=[sample_name],
                columns=[cytoband_names],
            )
        )
        ...
        for i in data.index:
            ...
            for cyto_i in cyto_chrom.index:
                ...
                if (data.loc[i]["START"] < cyto_chrom.loc[cyto_i]["chromEnd"]) | (
                    data.loc[i]["END"] < cyto_chrom.loc[cyto_i]["chromStart"]
                ):
                    # add MEAN_LOG2_COPY_RATIO value to corresponding cytoband column in cna_matrix
