...
for chrn in chrnlist:
    chrref[chrn] = pd.read_csv(
        os.path.join(dref, "GRCh38_gencode.v27.refFlat." + chrn + ".txt"),
        sep="\t",
        header=None,
    )
...
def generate_genedict(filepath):
    ...
    subid = filepath.split("/")[-1].split(".")[
        0
    ]  # find patient ID # FIX THIS PART if necessary
    data = data[
        data["CALL"] != "0"
    ]  # drop all rows where calls are not +/- # i think "0" should be string but double check
    for ix in data.index:
        ...
        for j in range(ref.shape[0]):
            ...
            # note - "start" "end" not necessarily the start/end point of transcription
            # it can be the reverse order depending on the strand
            # here, "start" is the position with the smaller genomic coord
            if (
                ...
                if (
                    ol_genename
                    in genedict.keys()  # pylint: disable=consider-iterating-dictionary
                ):  # pylint: disable=consider-iterating-dictionary
                    genedict[ol_genename].append(
                        copyratio
                    )  # add gene to list and store copy ratio value
                else:  # if gene name not in genelist:
                    genedict[ol_genename] = [
                        copyratio
                    ]  # add gene to list and store copy ratio value
            else:
                continue
...
def take_maxabs(genedict):  ## Take max/min value of amplification/deletion per gene
    newdict = {}
    for gene in genedict.keys():  # pylint: disable=consider-iterating-dictionary
...
def write_mx(genedict, subid, mx):
    ...
    if subid in mx.index:
        sys.exit(f"Error: subject {subid} already in dataframe. Cannot overwrite.")
    else:
        mx = pd.concat(
            [
                mx,
                pd.DataFrame(
                    np.zeros((1, mx.shape[1])), index=[subid], columns=mx.columns
                ),
            ]
        )
...
for filepath in glob.glob(os.path.join(din, "*.csv")):
    ...
    print(f"Done writing for file: {filepath}")
