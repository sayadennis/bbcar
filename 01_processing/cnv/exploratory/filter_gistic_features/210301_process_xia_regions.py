...
with open(
    "/Users/sayadennis/Projects/bbcar_project/bbcar_repository_staging/regions_xia.txt",
    "r",
) as f:
    lines = f.readlines()
...
reg = pd.DataFrame(
    None, index=[], columns=["#chrom", "chromStart", "chromEnd", "name", "amp_del"]
)
...
for line in lines:
    if line.startswith("chr"):
        ...
        reg = reg.append(
            pd.DataFrame(
                [[chrname, start, end, name, amp_del]], index=[1], columns=reg.columns
            )
        )
    else:
...
reg.to_csv(
    "/Users/sayadennis/Projects/bbcar_project/bbcar_repository_staging/regions_xia.tsv",
    sep="\t",
    header=True,
    index=False,
)
