...
"""
The purpose of this module is to provide the tools to filter 
the BBCar CNV matrices using a given region list.
The CNV matrix (cnv_mx):
    - pandas.DataFrame with index = subject IDs, and columns = [HUGO gene names | region name]
...
The region file (regions_fn) should: 
    ...
    - have 5 columns: #chrom, chromStart, chromEnd, name, amp_del.
    - amp_del should take values "+", "-", or "."
Reference file (ref_fn) can be two different things 
depending on whether features="genes" or features="regions:
    - ref_fn should be a string specifying an absolute or relative path to the file
    - if features="genes", ref_fn file should:
        - have any number of columns but the indices should be 
            - chromosome name: 2
            - gene name: 12
            - start pos: 4
            - end pos: 5
    - if features="regions", ref_fn file should:
        - be the file that documents our BBCar sample region boundaries
...
def filter_mx(cnv_mx, regions_fn, ref_fn, features="genes"):
    if features == "genes":
        ...
        for i in range(regions.shape[0]):
            ...
            gene_coor_sub = gene_coor.iloc[
                (gene_coor["chrom"] == chrn).values
            ]  # subset of the gene_coor where chromosome name matches the region of interest
            start_in_range = (start > gene_coor_sub["start"]) & (
                start < gene_coor_sub["end"]
            )
        ...
        print(
            (
                "Number of genes that mapped to the regions "
                f"specified by file {regions_fn}: {len(gene_names)}"
            )
        )
        ...
        print(
            (
                "Number of gene features in the CNV matrix "
                f"that survived: {filtered_cnv.shape[1]}\n"
            )
        )
        ...
        for i in range(ref.shape[0]):
            ref_chrn, ref_start, ref_end, ref_name = (
                ref.iloc[i, 0],
                ref.iloc[i, 1],
                ref.iloc[i, 2],
                ref.iloc[i, 3],
            ...
            for j in range(regions.shape[0]):
                ...
                if ref_chrn == reg_chrn:
                    if ((ref_start > reg_start) & (ref_start < reg_end)) | (
                        (ref_end > reg_start) & (ref_end < reg_end)
                    ):
                        if ref_name not in reg_names:
                            reg_names.append(ref_name)
                            print(amp_del_dict[ref_name])
        print(
            (
                "Number of peaks that overlapped with regions "
                f"specified by file {regions_fn}: {len(reg_names)}"
            )
        )
        # subset the cnv_mx based on the HUGO gene symbols in the remaining gene_regions
        col_bool = []
        ...
        print(
            f"Number of peaks in the CNV matrix that survived: {filtered_cnv.shape[1]}\n"
        )
        for colname in filtered_cnv.columns:
            if amp_del_dict[colname] == 1:
                for sample in (filtered_cnv[filtered_cnv[colname] < 0]).index:
...
def filter_by_genes(cnv_mx, gene_names_fn):
    ...
    print(f"Number of genes present in file {gene_names_fn}: {len(genelist)}")
    filtered_cnv = cnv_mx.iloc[:, [x in genelist for x in list(cnv_mx.columns)]]
    print("Number of genes that survived from matrix: {filtered_cnv.shape[1]}")
    return filtered_cnv
