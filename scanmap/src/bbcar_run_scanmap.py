...
opts, extraparams = getopt.getopt(sys.argv[1:], 'g:c:l:o:i:', 
                                  ['genomic=', 'confound=', 'label=', 'outdir=', 'index='])
...
# f = open('%s/%s/gene_thres_thres010_abs.pik' % (dn, config), 'rb')
# f = open(os.path.join(dn, 'reg_thres.pik'), 'rb')
f = open(genfn, 'rb')
genomic = pickle.load(f)
f.close()
# y = pd.read_csv(os.path.join(dn, 'bbcar_label_intindex.csv'), header=0, index_col=0)
...
# pts = pd.read_csv(os.path.join(dn, 'clin_prssnp_rm3.csv'))
pts = pd.read_csv(ptsfn)
...
print('nc,wcls,C,tr bal acc,val bal acc,te acc,te bal acc,te precis,te recall,te f1,w2,b2,celoss') # ,best iter,mse,mse tr,mse val,mse te
for nc in ncs:
    for C in [0.001, 0.01, 0.1, 1, 10, 100, 1000]: # tweak range if needed 
        for wcls in [0.01, 0.1, 0.5, 1, 2, 10, 50]: # tweak range if needed 
            ...
            fn = '%s/scanmap_k%d_wcls%s_C%s.p' % (outdir, nc, wcls, C) # scanmap%d/s%d/ # niter, seed, 
            m = ScanMap(
                np.vstack((X_train, X_val, X_test)), 
                cf = pts_tr, cfval = pts_val, y = y_train, yval = y_val, 
                k=nc, n_iter=4000, weight_decay=0, lr=lr, wcls=wcls, C=C, seed=2*722019+seed, fn=fn, device=device
            ...
            print('%d,%s,%s,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f' % 
                  (nc, wcls, C, acctr, accval, accte, balaccte, preciste, recallte, f1te, w2, b2, vce)) # best_iter, mse, mse_tr, mse_val, mse_te
