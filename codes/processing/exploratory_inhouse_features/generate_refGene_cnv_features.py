...
for chrn in chrnlist:
    chrref[chrn] = pd.read_csv(os.path.join(dref, "hg38_refGene_" + chrn + ".txt"), sep="\t", header=None)
    ...
    for ix in data.index:
        ...
        for j in range(ref.shape[0]):
            ...
            if (
                ...
                ol_genename = ref.iloc[j,12] # overlapping gene name. position 1 in refGene reference format
