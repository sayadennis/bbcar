...
#### KEGG ####
with open(os.path.join(reg_dn, 'genes_kegg_cancer.txt'), 'r') as f:
    lines = f.readlines()
...
print('Number of genes selected by KEGG: %s\n' % genecopy_kegg.shape[1])
# save 
genecopy_kegg.to_csv(os.path.join(out_dn, 'gene_copy_conf90_kegg_genes.csv'), header=True, index=True)
genethres_kegg.to_csv(os.path.join(out_dn, 'gene_thres_conf90_kegg_genes.csv'), header=True, index=True)

#### Dietlein cancer driver genes ####
dietlein_genes = []
with open(os.path.join(reg_dn, 'genes_dietlein_driver.txt'), 'r') as f:
    lines = f.readlines()
...
print('Number of genes selected by Dietlein: %s\n' % genecopy_dietlein.shape[1])
# save 
genecopy_dietlein.to_csv(os.path.join(out_dn, 'gene_copy_conf90_dietlein_genes.csv'), header=True, index=True)
genethres_dietlein.to_csv(os.path.join(out_dn, 'gene_thres_conf90_dietlein_genes.csv'), header=True, index=True)

#### Xia predictive genes ####
xia_genes = []
with open(os.path.join(reg_dn, 'genes_xia_predictive.txt'), 'r') as f:
    lines = f.readlines()
...
print('Number of genes selected by Xia: %s\n' % genecopy_xia.shape[1])
# save 
genecopy_xia.to_csv(os.path.join(out_dn, 'gene_copy_conf90_xia_genes.csv'), header=True, index=True)
genethres_xia.to_csv(os.path.join(out_dn, 'gene_thres_conf90_xia_genes.csv'), header=True, index=True)
