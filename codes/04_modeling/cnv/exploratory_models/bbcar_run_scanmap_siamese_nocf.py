...
from ScanMapSiamese_nocf import ScanMapSiamese_nocf
import torch
...
r = 0
if 'gene_' in genfn:
    ncs = range(50,501,50) # range(200,1001,200)
elif 'reg_' in genfn:
    ...
    for nc in ncs:
        for C in [0.01, 0.1, 1, 10, 100]: # , 0.1, 1, 10, 100, 1000, 0.001, 
            for wcls in [0.01, 0.1, 1]: # , 2, 10, 50, 0.5, 
                ...
                m = ScanMapSiamese_nocf(
                    np.vstack((X_train, X_val, X_test)), 
                    y = y_train, yval = y_val, 
                ...
                y_tr_pred = m.predict(X_tr_nmf)
                y_te_pred = m.predict(X_te_nmf)
