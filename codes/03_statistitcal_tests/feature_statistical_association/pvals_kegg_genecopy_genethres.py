...
genecopy = pd.read_csv("../../data/bbcar/genecopy_conf90_kegg.csv", index_col=0)
...
pval_df_sorted.head(10).to_csv("~/bbcar_project/indv_logistic/high_pval_genecopy_kegg.csv", index=True, header=True)


## Gene-level threshold values 

genethres = pd.read_csv("../../data/bbcar/genethres_conf90_kegg.csv", index_col=0)
label = pd.read_csv("../../data/bbcar/bbcar_label.csv", index_col=0)
...
for colname in genethres.columns:
    X = genethres[colname].to_numpy().reshape(-1,1)
    lmm = LinearRegression()
    lmm.fit(X, y)
    pvals.append(lmm.p[0][0])
    coefs.append(lmm.coef_[0][0])

pval_df = pd.DataFrame(np.array([coefs, pvals]).T, index=genethres.columns, columns=["coefs", "uncorrected p"])
pval_df_sorted = pval_df.iloc[np.argsort(pval_df["uncorrected p"].values),:]
...
pval_df_sorted.head(10).to_csv("~/bbcar_project/indv_logistic/high_pval_genethres_kegg.csv", index=True, header=True)
