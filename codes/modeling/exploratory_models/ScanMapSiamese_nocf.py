...
class ScanMapSiamese_nocf(nn.Module):
    def __init__(self, X, y = None, yval=None,
        ...
        self.__initfact__(X, y, yval)

    def __initfact__(self, X, y, yval):
        ...
        self.fc = nn.Linear(self.k*2, 4).to(self.device) # 4 = len(torch.unique(self.y))**2
    ...
    def to(self,device):
        ...
        return super(ScanMapSiamese_nocf, self).to(device)
    ...
    def fit(self):
        ...
        for e in it:
            ...
            if e >= self.n_iter - 1:
                y_val_pred = self.__predict__(self.Wval)
    ...
    def __predict__(self, w):
        ...
        y_pairs_fourclass = torch.argmax(self.fc(w_pairs), dim=1)
