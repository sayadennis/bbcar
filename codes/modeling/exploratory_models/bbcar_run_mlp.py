...
opts, extraparams = getopt.getopt(sys.argv[1:], 'g:l:o:i:c:n:', 
                                  ['genomic=', 'label=', 'outdir=', 'index=', 'contrastive=', 'n_iter='])
...
if 'reg_copy' in genfn:
    feature_type = 'regcopy'
elif 'reg_thres' in genfn:
    feature_type = 'regthres'
elif 'gene_copy' in genfn:
    feature_type = 'genecopy'
elif 'gene_thres' in genfn:
    feature_type = 'genethres'
elif 'pathway_avg' in genfn:
    feature_type = 'pathway_avg'
elif 'pathway_max' in genfn:
    feature_type = 'pathway_max'
elif 'pathway_med' in genfn:
    feature_type = 'pathway_med'
elif 'pathway_sum' in genfn:
    feature_type = 'pathway_sum'
else:
...
for lr in [0.0001, 0.001, 0.01]: # 0.00001, 0.0001 
    for C in [0.001, 0.01, 0.1, 1, 10, 100, 1000]: # 1000, 0.001 
        if contrastive:
            for wcont in [0.01, 0.1, 1, 10, 100]: # 1000, 0.001
                fn = '%s/bbcarmlp_%s_C%s_lr%s_wcont%s_niter%s_seed%s.p' % (outdir, feature_type, C, lr, wcont, niter, seed) # scanmap%d/s%d/ # niter, seed, 
                m = BBCarMLP(
                    X_train, X_val, y_train, y_val, 
                    n_iter=niter, hidden_layers=2, fn=fn, C=C, contrastive=contrastive, wcont=wcont, lr=lr, device=device # wcont = ,
                )
                m.fit()
                ...
                print('%s,%s,%s,%s,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f' % 
                    (C, lr, wcont, niter, acctr, accval, accte, balaccte, preciste, recallte, f1te, vce))
        else:
            fn = '%s/bbcarmlp_%s_C%s_lr%s_niter%s_seed%s.p' % (outdir, feature_type, C, lr, niter, seed) # scanmap%d/s%d/ # niter, seed, 
            m = BBCarMLP(
                X_train, X_val, y_train, y_val, 
                n_iter=niter, hidden_layers=2, fn=fn, C=C, lr=lr, device=device
            )
            m.fit()
