...
regthres = pd.read_csv("../../data/bbcar/reg_thres_conf90.csv", index_col=0).T
...
for colname in regthres.columns:
    X = regthres[colname].to_numpy().reshape(-1,1)
    lmm = LinearRegression()
    lmm.fit(X, y)
    pvals.append(lmm.p[0][0])
    coefs.append(lmm.coef_[0][0])

pval_df = pd.DataFrame(np.array([coefs, pvals]).T, index=regthres.columns, columns=["coefs", "uncorrected p"])
...
pval_df_sorted.head(40).to_csv("~/bbcar_project/indv_logistic/high_pval_regthres.csv", index=True, header=True)
